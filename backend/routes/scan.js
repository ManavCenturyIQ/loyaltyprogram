...
router.post("/users", async (req, res) => {
  try {
    ...
    const secondaryPoints = data.pass.recordData['members.member.secondaryPoints'];
    ...
    const user = new User({ name, email, mobile, passkitId, tierPoints, points, secondaryPoints, tierId, tierName, programId });
    await user.save();
    const token = user.generateToken();
    res.status(201).json({ token, user });
  } catch (error) {
...
router.post("/merchant/scan/:merchantSlug", auth, async (req, res) => {
  ...
  if (!merchantSlug || !passkitId) {
    return res.status(400).json({ message: "Missing merchant slug or user PassKit ID" });
  }
 ...
 console.log(user.email);
    ...
    if (user.updatedAt && now - new Date(user.updatedAt) < COOLDOWN_MS) {
      const secondsLeft = Math.ceil((COOLDOWN_MS - (now - user.updatedAt)) / 1000);
...
router.post('/admin/merchant', auth, admin, async (req, res) => {
  ...
  console.log(req.body);
    ...
    res.status(400).json({ message: 'Failed to create merchant', error: error.message });
  }
});
...
async function updateMemberTierPoints(newPoints, user) {
  try {
    ...
    console.error('Failed to update member tierPoints:', error.response?.data || error.message);
    throw error;
  }
}
